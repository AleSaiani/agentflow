...
function cmdResults(args) {
    ...
    const runId = positionals[0];
    if (!runId)
        die("error: results requires a run_id");
    const [cmd, state] = resolveOrExit(runId, values["cmd"]);
    let rows;
    if (cmd === "foreach") {
    ...
    for (const r of rows)
        process.stdout.write(`  ${String(r["id"]).slice(0, 64).padEnd(64)} ${r["status"] ?? ""}\n`);
}
function main(argv) {
    const [sub, ...rest] = argv;
    if (isHelp(sub))
        return printUsage("inspect", ["runs", "workflows", "history", "show", "results", "tree", "budget", "timeline", "board"]);
    switch (sub) {
        case "runs":
            return cmdRuns(rest);
        case "workflows":
