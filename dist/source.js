...
function parseLinePayload(payload) {
    ...
    if (m && m[1]) {
        for (const pair of m[1].split(",")) {
            ...
            if (key === "model")
                task.model = value;
            else if (key === "subagent" || key === "subagent_type" || key === "subagenttype")
                task.subagentType = value;
            else if (key === "prompt")
                task.prompt = value;
            else
                extra[key] = value;
...
export function loadSource(spec) {
    switch (spec.source) {
        case "inline":
            return spec.items;
        case "checkbox": {
            if (!existsSync(spec.path))
                die(`error: checkbox source not found at ${spec.path}`);
            return parseChecklist(readFileSync(spec.path, "utf8"));
        }
        case "file": {
            if (!existsSync(spec.path))
                die(`error: items file not found at ${spec.path}`);
...
export function writeFolderView(base, items) {
    ...
    for (const it of items)
        if (moveKanbanItem(base, it.id, it.data, it.status))
            moved++;
    return moved;
}
/**
 ...
 */
export function writeChecklistView(path, items) {
    if (!existsSync(path))
        die(`error: checkbox view target not found at ${path}`);
    const statusBySlug = new Map();
    const seen = new Map();
    for (const it of items) {
        // Recompute the same id slugging used by parseChecklist so we match the right line.
        const text = String(it.data?.["text"] ?? "");
        let id = slugify(text);
        const dup = seen.get(id);
        if (dup !== undefined) {
            seen.set(id, dup + 1);
