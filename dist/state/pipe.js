...
function validateStages(stagesRaw) {
    ...
    stagesRaw.forEach((raw, i) => {
        ...
        if (Array.isArray(next))
            for (const r of next)
                if (typeof r !== "object" || r === null || r["goto"] === undefined)
                    die(`error: stage ${i}: each branch rule needs a 'goto'`);
        out.push({
            index: i,
            name: raw["name"] ?? null,
            type: stype,
...
function cmdInit(args) {
    ...
    if (values["validate-only"])
        return print({ valid: true, cmd: CMD });
    ...
    for (const [name, decl] of Object.entries(paramSpec)) {
        ...
        let val = name in paramOverrides ? paramOverrides[name] : def;
        if (val === undefined) {
            if (obj && obj["required"])
        ...
        if (obj) {
            const enumVals = (obj["enum"] ?? obj["values"]);
            ...
            if (type === "number" || type === "integer") {
                ...
                if (s !== "true" && s !== "false")
                    die(`error: param '${name}'=${JSON.stringify(val)} must be true|false`);
                val = s === "true";
            }
        }
        params[name] = val;
...
function resolveTarget(state, target) {
    ...
    if (typeof target === "string") {
        const idx = state["stages"].findIndex((s) => s["name"] === target);
        if (idx < 0)
            die(`error: branch target '${target}' is not a stage name`);
        return idx;
    }
...
function nextIndex(state, cur) {
    ...
    if (Array.isArray(next)) {
        for (const ruleRaw of next) {
            ...
            const whenObj = typeof rule["when"] === "string" ? { type: "bash", command: rule["when"] } : rule["when"];
            if (evalWhenGuard(whenObj, state, cur["index"]) === 0)
                return resolveTarget(state, rule["goto"]);
        }
        return linear;
    }
...
function cmdApprove(args) {
    ...
    const runId = positionals[0];
    if (!runId)
        die("error: approve requires a run_id");
    const state = load(runId);
    const cur = currentStage(state);
    if (cur === null)
        die("error: no current stage to approve");
    if (!cur["approve"])
        die(`error: stage '${cur["name"] ?? cur["index"]}' is not an approval gate`);
    cur["approved"] = true;
    save(runId, state);
...
function childProgress(childCmd, childRunId) {
    ...
    if (childCmd === "foreach") {
        ...
        const c = { pending: 0, in_progress: 0, done: 0, failed: 0 };
        for (const it of items)
            c[it["status"]] = (c[it["status"]] ?? 0) + 1;
        return { kind: "foreach", total: items.length, ...c };
    }
    if (childCmd === "iterate")
        return { kind: "iterate", iter: s["iteration_count"] ?? 0, max: s["config"]?.max_iterations ?? null };
    if (childCmd === "group")
        return { kind: "group", groups: s["groups_count"] ?? 0, items: s["items_total"] ?? 0 };
    if (childCmd === "pipe") {
        const st = (s["stages"] ?? []);
        return { kind: "pipe", done: st.filter((x) => x["status"] === STATUS_DONE).length, total: st.length };
    }
...
function budgetTotal(cmd, runId) {
    ...
    const visit = (c, r) => {
        ...
        if (c === "pipe")
            for (const st of (s["stages"] ?? []))
                if (st["child_cmd"] && st["child_run_id"])
                    visit(st["child_cmd"], st["child_run_id"]);
    };
    visit(cmd, runId);
    return out;
}
...
function cmdProgress(args) {
    ...
    const runId = positionals[0];
    if (!runId)
        die("error: progress requires a run_id");
    const state = load(runId);
    ...
    const child = cur && cur["type"] === "primitive" && cur["child_cmd"] && cur["child_run_id"] ? childProgress(cur["child_cmd"], cur["child_run_id"]) : null;
    ...
    if (child && child["kind"] === "foreach")
        lines.push(`  └─ this phase: ${child["done"]}/${child["total"]} done${child["in_progress"] ? ` · ${child["in_progress"]} running` : ""}${child["failed"] ? ` · ${child["failed"]} failed` : ""}   ${bar(child["done"], child["total"])}`);
    else if (child && child["kind"] === "pipe")
        lines.push(`  └─ sub-workflow: ${child["done"]}/${child["total"]} stages   ${bar(child["done"], child["total"])}`);
    else if (child && child["kind"] === "iterate")
        lines.push(`  └─ iteration ${child["iter"]}/${child["max"] ?? "?"}`);
    else if (child && child["child_status"])
        lines.push(`  └─ ${child["kind"]}: ${child["child_status"]}`);
    lines.push(`  scale: ${bud.agents} agents · ~$${usd.toFixed(2)}${running ? ` · resume ${state["auto_continues"] ?? 0}${maxC ? `/${maxC}` : ""}` : ""}`);
    if (running) {
        const remaining = stages.filter((s) => s["status"] !== STATUS_DONE && s["status"] !== STATUS_SKIPPED).map((s) => s["name"]);
        if (remaining.length)
            lines.push(`  next: ${remaining.slice(0, 4).join(" → ")}${remaining.length > 4 ? " → …" : ""}`);
    }
...
function hasResidualWork(state) {
    ...
    if (curStatus === STATUS_PENDING && cur["approve"] && cur["approved"] !== true)
        return null;
...
function main(argv) {
    const [sub, ...rest] = argv;
    if (isHelp(sub))
        return printUsage(CMD, ["init", "tick", "drive", "plan", "progress", "complete-bash-stage", "complete-json-stage", "start-primitive-child", "advance", "approve", "fail", "status", "runs", "increment-continues", "budget-add"]);
    switch (sub) {
        case "init":
            return cmdInit(rest);
        case "tick":
