 ...
 */
function folderBaseFromValues(values) {
    const folder = values["folder"];
    if (folder)
    ...
    if (sourceJson) {
        try {
            ...
            if (spec && spec.source === "folder" && typeof spec.path === "string")
...
function syncKanban(state, item) {
    const base = state["config"]?.["folder"];
    if (!base)
        return;
...
function cmdInit(args) {
    ...
    const shardStr = values["shard"];
    let shard = null;
    if (shardStr) {
        const m = shardStr.match(/^(\d+)\s*\/\s*(\d+)$/);
        if (!m)
            die(`error: --shard must be k/N (e.g. 0/2), got '${shardStr}'`);
        const k = parseInt(m[1], 10);
        const n = parseInt(m[2], 10);
        if (n < 1 || k < 0 || k >= n)
            die(`error: --shard requires 1<=N and 0<=k<N, got ${k}/${n}`);
        shard = { k, n };
    }
    ...
    if (shard)
        sourceItems = sourceItems.filter((_, idx) => idx % shard.n === shard.k);
    const items = {};
    ...
    if (promptFile && (values["prompt"] !== undefined || values["task-prompt"]))
    ...
    if (promptFile) {
        ...
        effectiveTaskPrompt = (values["prompt"] ?? values["task-prompt"]) || "";
    }
    ...
    const concurrency = serial ? 1 : parseInt(values["concurrency"], 10);
    const chunkSize = serial ? "1" : values["chunk-size"];
...
function cmdClaim(args) {
    ...
    for (const item of claimed)
        syncKanban(state, item); // todo/ → in-progress/
    save(runId, state);
    print(claimed);
}
/**
...
function hasResidualWork(state) {
    ...
    if (stopFile && existsSync(String(stopFile)))
        return null;
    const items = Object.values(state["items"] ?? {});
...
function resumeMsg(runId, residual) {
    ...
    return (`/agentflow:foreach run '${runId}' is not complete: ${pending} pending, ${inProgress} in_progress. ` +
