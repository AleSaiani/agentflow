...
min_items: 15               # autonomous-invocation guardrail: if Claude triggers /foreach but resolves < min_items, ask the user to confirm (avoids orchestration overhead for tiny batches). Bypassed when the user types /foreach explicitly.
---
...
- `min_items: 15` is the threshold below which an *autonomous* /foreach invocation
  triggers a confirmation prompt. Tune up if you find /foreach firing too eagerly
