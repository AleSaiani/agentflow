...
- This recipe does NOT modify any files. Outputs land under `.agentflow/audit/<run-id>/`
  (the pipe run's directory) and the per-file review state under `.agentflow/foreach/<run-id>-s1-foreach/`.
