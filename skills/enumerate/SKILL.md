...
description: |
  ...
  DON'T use when the list already exists (a file, a glob, a markdown checklist) — point /agentflow:foreach at
  it directly; or when you need a single synthesized output (→ /agentflow:reduce).
  Explicit: `/agentflow:enumerate --prompt "<what to produce>" [--input <source>]`.
allowed-tools: Bash, Read, Write, Agent
argument-hint: --prompt "<instructions>" [--input <path>] [--model haiku|sonnet|opus] [--execution main-thread|subagent]
---

# /agentflow:enumerate — unfold (1 → N)
...
- `/agentflow:foreach --items <file>` to apply an operation to each item;
- `/agentflow:group --input-source <descriptor>` to partition them;
- inside a `/agentflow:pipe`, wire `{{stages.<this-stage>.result_pointer}}` into the next stage's input.
