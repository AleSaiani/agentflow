---
# Default config for /agentflow:group.
#
...
auto_continue: true         # if true, the Stop hook resumes a half-done /agentflow:group in the next turn
max_auto_continues: 5       # safety cap; deterministic methods complete in one shot, > 1 is recovery
min_items: 10               # autonomous-invocation guardrail (Claude triggers /agentflow:group only if >= min_items)
...
- /agentflow:group is the **partition** primitive. Output is items.json-compatible: each "item"
  in the output is a group, ready to be consumed by /agentflow:foreach as `--items`.
  This is the canonical `/agentflow:group → /agentflow:foreach (per group)` composition.
...
- `min_items: 10` is the autonomous-invocation guardrail. For very small input sets,
  ...
  when the user types `/agentflow:group` explicitly.
