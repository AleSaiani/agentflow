...
1. Every run writes its state to a file on disk (`.agentflow/foreach/<id>/state.json`, etc.).
...
- The state dirs (`.agentflow/foreach/`, `.agentflow/pipe/`, `.agentflow/audit/`, …) appear only when a skill actually ran.
