...
/** Run the enumerate CLI as a subprocess; returns parsed JSON stdout (last line). */
function run(stateDir: string, args: string[]): any {
  const out = execFileSync("node", [CLI, ...args], {
    encoding: "utf8",
    env: { ...process.env, ENUMERATE_STATE_DIR: stateDir },
  });
  const lines = out.trim().split(/\r?\n/).filter(Boolean);
  return JSON.parse(lines[lines.length - 1] ?? "null");
}

test("enumerate CLI: checkbox init pre-marks [x], claim/complete drives to done", () => {
...
test("enumerate CLI: complete-batch + fail/retry + reset", () => {
  const dir = mkdtempSync(join(tmpdir(), "enum-cli-"));
...
test("enumerate CLI: validate-only does not read items or write state", () => {
  const dir = mkdtempSync(join(tmpdir(), "enum-cli-"));
