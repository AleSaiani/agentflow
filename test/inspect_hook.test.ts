...
test("Stop hook blocks on an active enumerate run and pre-increments auto_continues", () => {
  const { env } = freshEnv();
  ...
  assert.match(decision.reason, /enumerate run 'r1'/);

  // pre-increment persisted
  const state = JSON.parse(readFileSync(join(env["ENUMERATE_STATE_DIR"]!, "r1", "state.json"), "utf8"));
...
test("inspect runs/board surface the active run", () => {
  ...
  assert.equal(runs[0].cmd, "enumerate");
  ...
  assert.equal(show.cmd, "enumerate");
