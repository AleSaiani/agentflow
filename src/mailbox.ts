...
function boxesRoot(id: string): string {
  return join(stateDir("mailbox"), id);
}
function boxDir(id: string, box: string): string {
...
function cmdSend(args: string[]): void {
  ...
  if (!id || !to) die("error: send requires a run_id and --to <box>");
  let body: unknown;
  if (values["json"] !== undefined) body = JSON.parse(values["json"] as string);
  else if (values["message-file"]) body = readFileSync(values["message-file"] as string, "utf8");
  else if (values["message"] !== undefined) body = values["message"];
  else die("error: send requires --message, --message-file, or --json");
  ...
  const msg = { to, from: (values["from"] as string) ?? null, ts, body };
...
function cmdRecv(args: string[]): void {
  const { values, positionals } = parseArgs({ args, allowPositionals: true, strict: true, options: { box: { type: "string" } } });
  const id = positionals[0];
  const box = values["box"] as string | undefined;
  if (!id || !box) die("error: recv requires a run_id and --box <box>");
...
function cmdPeek(args: string[]): void {
  const { values, positionals } = parseArgs({ args, allowPositionals: true, strict: true, options: { box: { type: "string" } } });
  const id = positionals[0];
  const box = values["box"] as string | undefined;
  if (!id || !box) die("error: peek requires a run_id and --box <box>");
  const dir = boxDir(id, box);
  const pending = existsSync(dir) ? readdirSync(dir).filter((f) => f.endsWith(".json")).sort() : [];
...
function cmdStatus(args: string[]): void {
  ...
  const id = positionals[0];
  if (!id) die("error: status requires a run_id");
  const root = boxesRoot(id);
  const boxes: Record<string, { pending: number; read: number }> = {};
  if (existsSync(root)) {
