...
function resolveRun(idArg: string, cmdFilter?: string): [string, string] {
  if (idArg.includes("/")) {
    const [c, r] = splitRunKey(idArg);
    if (!existsSync(statePath(c, r))) die(`error: no run at ${c}/${r}`);
    return [c, r];
  }
  if (cmdFilter) {
    if (!existsSync(statePath(cmdFilter, idArg))) die(`error: no run '${idArg}' under ${cmdFilter}`);
    return [cmdFilter, idArg];
  }
  const matches = [...PRIMITIVES.keys()].filter((c) => existsSync(statePath(c, idArg)));
  if (matches.length === 0) die(`error: no run found with id '${idArg}'`);
  if (matches.length > 1) die(`error: run-id '${idArg}' is ambiguous (matches ${matches.join(", ")}); use <cmd>/${idArg}`);
  return [matches[0] as string, idArg];
}

/** Invert childToParent into parent→[children] for subtree walks. */
function parentToChildren(childMap: Map<string, string>): Map<string, string[]> {
...
function subtreeKeys(key: string, inv: Map<string, string[]>): string[] {
  const out: string[] = [];
  ...
  const walk = (k: string): void => {
    ...
    seen.add(k);
    out.push(k);
    for (const c of inv.get(k) ?? []) walk(c);
  };
  walk(key);
  return out;
}

function runDir(cmd: string, runId: string): string {
  return dirname(statePath(cmd, runId));
}

// ---------- presentation helpers ----------

function progressOf(cmd: string, s: StateDict): string {
  if (cmd === "foreach") {
  ...
  if (cmd === "pipe") {
    const stages = (s["stages"] ?? []) as StateDict[];
    const done = stages.filter((x) => x["status"] === "done").length;
    return `${done}/${stages.length} stages`;
  }
  if (cmd === "iterate") return `${s["iteration_count"] ?? 0}/${s["config"]?.max_iterations ?? "?"} iters`;
  if (cmd === "group") return `${s["groups_count"] ?? 0} groups`;
  return "—";
}

/** Effective status for display: global pause and per-job pause take precedence over the raw status. */
function effectiveStatus(cmd: string, s: StateDict, globally: boolean): string {
  const spec = getPrimitive(cmd);
  const raw = String(s["status"] ?? "?");
  if (TERMINAL.has(raw)) return raw;
  if (spec && spec.isDone(s)) return "done";
  if (globally) return "paused (engine)";
  const [paused, reason] = isPaused(s);
  if (paused) return s["paused"] === true ? "paused" : `paused (${reason})`;
  return raw;
}
...
function parseDuration(spec: string): number | null {
  ...
  return n * (mult[unit] as number);
}

function ageMs(s: StateDict): number {
...
function loadTopLevelJobs(): JobRow[] {
...
function cmdList(args: string[]): void {
  ...
  schedulable.sort(
    (a, b) => b.priority - a.priority || a.created_at.localeCompare(b.created_at) || a.reg - b.reg || a.run_id.localeCompare(b.run_id),
  );
  const pos = new Map<string, number>();
  if (!globally) schedulable.forEach((j, i) => pos.set(j.key, i + 1));
  ...
  const lines: string[] = [];
  if (globally) lines.push("⏸  ENGINE PAUSED — no run auto-resumes. `runs resume` to unfreeze.\n");
  if (jobs.length === 0) {
  ...
  for (const j of jobs) {
    ...
    lines.push(
      `${(p ? `#${p}` : "·").padEnd(4)} ${j.cmd.padEnd(10)} ${j.run_id.slice(0, 34).padEnd(34)} ${effectiveStatus(j.cmd, j.state, globally).padEnd(18)} ${progressOf(j.cmd, j.state).padEnd(16)} ${String(j.priority).padEnd(5)} ${j.state["updated_at"] ?? ""}`,
    );
  }
...
function cmdStop(args: string[]): void {
  ...
  if (!idArg) die("error: stop requires a run-id (or use `runs pause` to pause the whole engine)");
  const [cmd, runId] = resolveRun(idArg, values["cmd"] as string | undefined);
...
function cmdPause(args: string[]): void {
  ...
  if (positionals[0]) die("error: `runs pause` is global (takes no id). To pause one job use `runs stop <id>`.");
...
function cmdRm(args: string[]): void {
  ...
  const idArg = positionals[0];
  if (!idArg) die("error: rm requires a run-id");
  const [cmd, runId] = resolveRun(idArg, values["cmd"] as string | undefined);
  const key = `${cmd}/${runId}`;
  const childMap = childToParent();
...
function cmdClean(args: string[]): void {
  ...
  const wanted = new Set<string>(["done"]);
  if (values["failed"] || values["all"]) wanted.add("failed");
  if (values["all"]) wanted.add("aborted");

  let minAge = 0;
  if (values["older-than"]) {
    const parsed = parseDuration(values["older-than"] as string);
    if (parsed === null) die(`error: --older-than must look like 30m, 24h, 7d, 2w (got '${values["older-than"]}')`);
    minAge = parsed;
  }

  const childMap = childToParent();
  const inv = parentToChildren(childMap);
  const selected: { key: string; status: string }[] = [];
  for (const job of loadTopLevelJobs()) {
    ...
    if (minAge && ageMs(job.state) < minAge) continue;
  ...
  print({
    ...
    statuses: [...wanted],
    older_than: (values["older-than"] as string) ?? null,
  });
}
...
function cmdPriority(args: string[]): void {
  ...
  if (!idArg || nArg === undefined) die("error: priority requires <run-id> <n> (integer; higher runs sooner)");
  const n = parseInt(nArg, 10);
  if (!Number.isFinite(n)) die(`error: priority must be an integer, got '${nArg}'`);
  const [cmd, runId] = resolveRun(idArg, values["cmd"] as string | undefined);
  const p = statePath(cmd, runId);
  const state = loadState(p);
  state["priority"] = n;
...
function main(argv: string[]): void {
  const [sub, ...rest] = argv;
  if (isHelp(sub)) return printUsage("runs", ["list", "stop", "resume", "pause", "rm", "clean", "priority"]);
  switch (sub) {
    case "list":
    case "ls":
      return cmdList(rest);
