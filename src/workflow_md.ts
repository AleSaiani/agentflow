...
function parseFlow(s: string): unknown {
  ...
  if (t.startsWith("[")) return splitTop(t.slice(1, -1)).map((p) => parseScalar(p));
...
function parseFrontmatter(lines: string[]): Frontmatter {
  ...
  if (end < 0) return { meta: {}, rest: lines };
  ...
  for (let i = 0; i < fm.length; i++) {
    ...
    if (indent > 0) continue; // consumed by the nested collector below
    ...
    for (; j < fm.length; j++) {
      ...
      if (!sm) break;
      obj[sm[2] as string] = parseScalar(sm[3] ?? "");
    }
    if (Object.keys(obj).length) meta[key] = obj;
    i = j - 1;
  }
...
function collectBody(body: string[]): StageBody {
  ...
  for (const line of body) {
    ...
    if (b) bullets.push([b[1] as string, parseScalar(b[2] ?? "")]);
  }
  return { fences, bullets };
}

function bulletGet(bullets: Array<[string, unknown]>, key: string): unknown {
  for (const [k, v] of bullets) if (k === key) return v;
  return undefined;
}
...
function compileStage(s: RawStage): Record<string, unknown> {
  ...
  if (s.type === "bash") {
    ...
    if (cmd !== undefined) spec["command"] = cmd.trim();
    const out = bulletGet(bullets, "output_path") ?? bulletGet(bullets, "output-path");
    if (out !== undefined) spec["output_path"] = String(out);
    stage["spec"] = spec;
  } else if (s.type === "json") {
    stage["type"] = "json";
    const spec: Record<string, unknown> = {};
    const v = bulletGet(bullets, "value");
    if (v !== undefined) spec["value"] = v;
    else if (fences["json"] !== undefined) spec["value"] = JSON.parse(fences["json"]);
    const out = bulletGet(bullets, "output_path") ?? bulletGet(bullets, "output-path");
    if (out !== undefined) spec["output_path"] = String(out);
    stage["spec"] = spec;
    ...
    stage["type"] = s.type;
    const spec: Record<string, unknown> = {};
    for (const [k, v] of bullets) if (k !== "when") spec[k] = v;
    stage["spec"] = spec;
  }
  return stage;
}
...
export function parseWorkflowMd(text: string): Record<string, unknown> {
  const lines = text.split(/\r?\n/);
  const { meta, rest } = parseFrontmatter(lines);
  const stages: Record<string, unknown>[] = [];
  let cur: RawStage | null = null;
  const flush = (): void => {
    if (cur) stages.push(compileStage(cur));
    cur = null;
  };
  for (const line of rest) {
    ...
    if (h) {
      flush();
      cur = { name: (h[1] ?? "").trim(), type: h[2] as string, body: [] };
      continue;
    }
    if (cur) cur.body.push(line);
  }
  flush();
  const spec: Record<string, unknown> = { stages };
  for (const k of ["name", "description", "params", "config"]) if (k in meta) spec[k] = meta[k];
  return spec;
}
