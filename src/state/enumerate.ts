...
function cmdInit(args: string[]): void {
  ...
  const taskPrompt = ((values["prompt"] ?? values["task-prompt"]) as string) || "";
  if (!taskPrompt) die("error: --prompt (the generation instructions) is required");
  ...
  const state = makeBaseState(
    ...
    { task_prompt: taskPrompt, input: (values["input"] as string) ?? null },
  );
...
function cmdComplete(args: string[]): void {
  ...
  const runId = positionals[0];
  const itemsPath = values["items-path"] as string | undefined;
  if (!runId || !itemsPath) die("error: complete requires run_id and --items-path");
  if (!existsSync(itemsPath)) die(`error: items file does not exist at ${itemsPath}`);
  const parsed = JSON.parse(readFileSync(itemsPath, "utf8"));
  if (!Array.isArray(parsed)) die("error: the generated items file must be a JSON array");
  const state = load(runId);
...
function resumeMsg(runId: string, residual: ResidualWork): string {
  ...
  return (
    `/agentflow:enumerate run '${runId}' is not complete (status=${status}). ` +
...
function main(argv: string[]): void {
  const [sub, ...rest] = argv;
  if (isHelp(sub)) return printUsage(CMD, ["init", "start", "complete", "fail", "status", "runs", "increment-continues", "budget-add"]);
  switch (sub) {
    case "init":
      return cmdInit(rest);
    case "start":
