...
import { runBash, type ShellResult } from "../shell.js";
...
function killPath(runId: string): string {
  return join(stateDir(CMD), runId, "kill");
}
function iterDir(runId: string): string {
  return join(stateDir(CMD), runId);
}

/** Accept a step as a plain bash command string OR a JSON object `{type, command, mode?}`. */
function coerceStep(raw: string): StateDict {
  const t = raw.trim();
  if (t.startsWith("{")) {
    try {
      const obj = JSON.parse(t);
      if (obj && typeof obj === "object" && !Array.isArray(obj)) return obj as StateDict;
    } catch {
...
function cmdInit(args: string[]): void {
  ...
  const stage = coerceStep(values["stage"] as string);
  const mode = values["mode"] as string;
  if (!["until", "while"].includes(mode)) die("error: --mode must be 'until' or 'while'");
  let stop: StateDict;
  if (values["stop"]) {
    stop = coerceStep(values["stop"] as string);
    if (!stop["mode"]) stop["mode"] = mode;
  } else if (times !== null) {
...
function cmdRunIteration(args: string[]): void {
  ...
  let prevOut = "";
  if (iterIndex > 0) prevOut = (state["iterations"].at(-1)?.["stage_output_path"] as string) ?? "";
...
function hasResidualWork(state: StateDict): ResidualWork | null {
  ...
  if (status === STATUS_PENDING || status === STATUS_IN_PROGRESS) return [status, state["iteration_count"] ?? 0];
  return null;
}
function resumeMsg(runId: string, residual: ResidualWork): string {
  const [status, iterCount] = residual as [string, number];
  return (
    `/iterate run '${runId}' is not complete (status=${status}, iterations so far=${iterCount}). ` +
...
function main(argv: string[]): void {
  const [sub, ...rest] = argv;
  if (isHelp(sub)) return printUsage(CMD, ["init", "run-iteration", "kill", "fail", "status", "runs", "increment-continues", "budget-add"]);
  switch (sub) {
    case "init":
      return cmdInit(rest);
    case "run-iteration":
