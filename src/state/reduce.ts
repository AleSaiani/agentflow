...
function cmdInit(args: string[]): void {
  ...
  const inputsPath = values["inputs"] as string | undefined;
  if (!inputsPath) die("error: init requires --inputs");
  const inputs = JSON.parse(readFileSync(inputsPath, "utf8"));
  if (!Array.isArray(inputs)) die("error: inputs file must contain a JSON array of input descriptors");
  inputs.forEach((inp: Record<string, unknown>, i: number) => {
    if (typeof inp !== "object" || inp === null || !("source" in inp))
      die(`error: input at index ${i} must be an object with 'source'`);
    if (!["run", "file", "inline"].includes(inp["source"] as string))
...
function cmdComplete(args: string[]): void {
  ...
  if (!runId || !outputPath) die("error: complete requires run_id and --output-path");
  if (!existsSync(outputPath)) die(`error: output file does not exist at ${outputPath}`);
  const state = load(runId);
...
function cmdMaterialize(args: string[]): void {
  const { values, positionals } = parseArgs({
    args,
    allowPositionals: true,
    strict: true,
  ...
  const runId = positionals[0];
  const outArg = values["out"] as string | undefined;
  if (!runId || !outArg) die("error: materialize requires run_id and --out");
  const state = load(runId);
  const out: StateDict = { inputs: [] };
  for (const inp of state["inputs"] as StateDict[]) {
    ...
    if (src === "run") {
      ...
      const entry: StateDict = { source: "run", cmd: otherCmd, run_id: otherRun, items: [] };
      for (const item of Object.values(otherState["items"] ?? {}) as StateDict[]) {
        entry["items"].push({
          id: item["id"],
          status: item["status"],
  ...
  const totalItems = (out["inputs"] as StateDict[])
...
function resumeMsg(runId: string, residual: ResidualWork): string {
  ...
  return (
    `/reduce run '${runId}' is not complete (status=${status}). ` +
...
function main(argv: string[]): void {
  const [sub, ...rest] = argv;
  if (isHelp(sub)) return printUsage(CMD, ["init", "start", "complete", "fail", "status", "materialize", "runs", "increment-continues", "budget-add"]);
  switch (sub) {
    case "init":
      return cmdInit(rest);
    case "start":
