...
export function whichBash(): string {
  ...
  if (override && existsSync(override)) return (cachedBash = override);

  if (!isWin) {
    for (const dir of (process.env["PATH"] || "").split(":"))
      if (dir && existsSync(join(dir, "bash"))) return (cachedBash = join(dir, "bash"));
  ...
  for (const dir of (process.env["PATH"] || "").split(";")) {
    ...
    for (const e of ["bash.exe", "bash"]) {
      const c = join(dir, e);
      if (existsSync(c)) return (cachedBash = c);
    }
  }

  // Known Git for Windows install locations.
  for (const base of [process.env["PROGRAMFILES"], process.env["PROGRAMFILES(X86)"], process.env["LOCALAPPDATA"] && join(process.env["LOCALAPPDATA"] as string, "Programs")]) {
    if (!base) continue;
