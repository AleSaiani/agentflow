...
| **[remediate](remediate/WORKFLOW.md)** | Close the loop after a review/checklist: apply a fix for **every** finding → loop a verify command until it's green | `findings`/`checklist`, `min_severity`, `verify_cmd`, `max_rounds` | foreach fix + an `iterate` (until) verify loop — the dev-cycle complement to pr-review |
