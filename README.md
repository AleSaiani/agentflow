...
[![version](https://img.shields.io/badge/version-1.0.0--beta.8-blue)](CHANGELOG.md)
...
`1.0.0-beta.8`. Primitives, the workflow layer (WORKFLOW.md, params, fork, schema validation, stage
retry/timeout, approval gate, on-failure), `queue`/`mailbox`/`step`, inspect/board, and the hooks
(cross-turn resume + preserve-chat) are implemented and covered by an automated suite (**126 tests**, run
